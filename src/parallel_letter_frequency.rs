...
pub fn frequency(input: &[&str], worker_count: usize) -> HashMap<char, usize> {
    ...
    let rem = input.len() % worker_count;
    let part_size = input.len() / worker_count;
    let part_size = if rem > 0 { part_size + 1 } else { part_size };

    thread::scope(|s| {
        input.chunks(part_size).for_each(|chunk| {
            s.spawn(|| {
                chunk
                    .iter()
                    .flat_map(|x| x.chars())
