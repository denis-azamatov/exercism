   ...
   ($($name:ident = $discriminant:literal),* $(,)?) => {
       ...
       impl Allergen {
           fn from_u32(value: u32) -> Option<Self> {
               match value {
                   $($discriminant => Some(Allergen::$name), )*
                   _ => None
               }
           }
       }
...
impl Allergies {
    ...
    pub fn is_allergic_to(&self, allergen: &Allergen) -> bool {
        let discriminant = *allergen as u32;
        self.score & discriminant == discriminant
    }

    pub fn allergies(&self) -> Vec<Allergen> {
        (0..u32::BITS)
            .filter_map(|i| Allergen::from_u32(self.score & 1 << i))
            .collect()
    }
}
